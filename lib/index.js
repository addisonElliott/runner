      ...
      return new Promise(function(resolve, reject) {
        ...
        proc.on('message', function(message) {
          ...
          if (message.event === 'started') {
            ...
            debug('okay, here\'s the error and all', message);
            reject(message.err);
          }
        });
        ...
        proc.on('error', function(err) {
          debug('got an error here 222!', err);
          reject(err);
        });
...
function stop(opts) {
  ...
  return util.promisify(require('kill-mongodb'));
}
...
function configure(opts) {
  return new Promise(function(resolve, reject) {
    ...
    if (opts.action === 'stop') {
      ...
      mkdirp(opts.pidpath)
        .then(getDbPath.bind(null, opts))
        .then(createLogsDirectory.bind(null, opts))
        .then(resolve)
        .catch(reject);
    }
  });
...
var exec = (module.exports = exports = function(opts) {
  return new Promise(function(resolve, reject) {
    ...
    if (opts.action !== 'stop') {
      ...
      promise = util.promisify(mvm.use)({
    ...
    promise.then(configure.bind(null, opts)).then(function() {
